# Exercises 6a / 6b / 6c — Instructor Guide

Covers all three challenges in the cost series. Reveals the answers —
don't hand this to learners.

  - **6a** `cost-timers` — measure the baseline, replace the polling loop
  - **6b** `cost-payloads` — move the evidence out of Workflow History
  - **6c** `cost-heartbeats` — heartbeat frequency and the timeout that drives it

Each challenge's practice/ starts from the previous challenge's solution/,
the same way exercises 1-5 chain.

## What this series is for

Exercise 4 taught one cost lever (Local Activities for fast in-process steps).
These three teach the levers that usually matter more — **payload size**, **timer
...
|---|---|---|---|
| history bytes (`workflow show --output json`) | 3,758,007 | 585,746 | −84% |
| total events | 353 | 52 | −85% |
| `ACTIVITY_TASK_SCHEDULED` | 8 | 7 | −1 |
| `TIMER_STARTED` | 60 | 1 | −98% |
| heartbeat RPCs | 8 | 1 | −88% |
...
`heartbeat()` 30 times but produces only 8 RPCs. The SDK throttles heartbeats to
roughly one per 80% of the heartbeat timeout — at a 5s timeout that's one every 4s,
and 8 across a 30s Activity.

Both of these are measured, not reasoned:

  - raising the timeout to 60s **alone**, still heartbeating every step, gives 1 RPC
  - the full fix (guard + timeout) also gives 1 RPC

So the timeout is necessary and sufficient for the cost saving, and the guard
contributes nothing to it. Don't oversell the guard — its value is that it stops the
code depending on an SDK throttle nobody chose, and it makes the checkpoint interval
explicit so the longer timeout is reviewable.

The coupling runs the other way and is worth triggering live if you have time: set
`HEARTBEAT_EVERY_N_STEPS = 10` and leave `HEARTBEAT_TIMEOUT_SECONDS = 5`. Heartbeats
10s apart against a 5s timeout means the Activity dies mid-analysis.
...
After 6a the appeal window still elapses in full unless signalled, so runs stay
~95s throughout the series.
...
- **Filter the heartbeat metric to one service.** The server emits a
  `service_requests` row for `service_name="frontend"` *and* one for
  `service_name="history"` covering the same RPCs. Summing both double-counts. The
  assignment's grep filters to `frontend`; if someone drops that filter they'll see
  16 where the table says 8.
- **To see a heartbeat live**, run `temporal workflow describe` while the analysis
  Activity is in flight: `pendingActivities[0]` carries `heartbeatDetails`,
  `lastHeartbeatTime`, and `attempt`. It disappears the moment the Activity
  completes — the details live in mutable state, not in history.
...
## Per-challenge measurements

Numbers a learner should see at the end of each challenge:

| | baseline | after 6a | after 6b | after 6c |
|---|---|---|---|---|
| history bytes | 3,758,007 | 3,636,196 | 585,745 | 585,746 |
| total events | 353 | 58 | 52 | 52 |
| activity tasks | 8 | 8 | 7 | 7 |
| timers | 60 | 1 | 1 | 1 |
| heartbeat RPCs | 8 | 8 | 8 | 1 |

Every column measured on a real dev-server run, one claim each.

Two of these surprise people, so be ready for them:

  - **6a cuts events by 84%, not by 59.** Each timer tick also woke the Workflow, so
    every iteration cost a TimerStarted, a TimerFired, and a WorkflowTask triplet —
    about five events per tick. 6a barely touches history *size*, though, because
    the evidence is untouched.
  - **Heartbeat RPCs are 8, not 30.** See finding 5 below.

## Why this is three challenges and not one

It was one, and it was too much for a single sitting. Splitting also lets each
fix be measured on its own, which is what makes the 6b "activity count barely
moved" and 6c "the timeout did the work, not your guard" moments land — in a
single challenge those get lost in one combined before/after.
